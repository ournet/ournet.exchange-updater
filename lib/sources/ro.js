...
function postRates(rates) {
	rates = rates.filter(function(rate) {
		rate.source = exchange.data.getSourceByUniqueName(country, rate.source.replace(/(-bank)?-\d+$/i, ''));
...
function addSources() {
	...
	for (var i = currencyIds.length - 1; i >= 0; i--) {
		...
		var multiplier = cid[1].length > 3 ? parseInt(cid[1].substr(0, cid[1].length - 3)) : 1;
		var currency = cid[1].substr(cid[1].length - 3);
		sources.push({
			name: 'business24.ro',
			options: {
				url: 'http://www.business24.ro/index.php?module=AmvcCVBanks&id=' + cid[0] + '&date=' + dayDate,
