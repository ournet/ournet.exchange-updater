...
module.exports = [{
	...
	options: {
		...
		headers: headers,
		dateReg: /<h2>Курсове на българския лев към отделни чуждестранни валути и цена на златото, валидни за\s+(\d+\.\d+\.\d{4})\s*<\/h2>/i,
	...
	options: {
		...
		reg: /<tr(?:[^>]*)>\s*<td(?:[^>]*)>\s*<a(?:[^>]*)>\s*(\w{3})\s*<\/a>\s*<\/td>\s*<td[^>]*>[^<]+<\/td>\s*<td[^>]*>\s*(\d+)\s*<\/td>\s*<td[^>]*>[^<]+<\/td>\s*<td[^>]*>\s*([\d\.]+)\s*<\/td>\s*<td[^>]*>\s*([\d\.]+)\s*<\/td>/gi,
	...
	options: {
		...
		reg: /<tr[^>]*>\s*<td[^>]*>\s*<a href="\/bg\/valutni-kursove\/(\w{3})\/"[^>]*>\s*<span[^>]+>[^<]*<\/span>[^<]*<\/a>[^<]+<\/td>\s*<td[^>]*>\s*(\d+)\s*<\/td>\s*<td[^>]*>([\d\.,]+)<\/td>\s*<td[^>]*>\s*([\d\.,]+)\s*<\/td>/gi,
		names: ['currency', 'multiplier', 'buy', 'sale'],
		headers: headers,
		dateReg: /<input[^>]* value="(\d{2}\/\d{2}\/\d{4})"/g,
		dateParser: function (date) {
			date = date.split(/\//g);
			date.reverse();
			return date.join('-');
...
// 	source: 105,
// 	options: {
// 		url: 'https://dskbank.bg/restapi/dsk/GetCurrencyRates/',
...
// },
{
	...
	options: {
		...
		reg: /<tr[^>]*>\s*<td[^>]*>\s*(\w{3})\s*<\/td>\s*<td[^>]*>\s*(\d+)\s*<\/td>\s*<td[^>]*>\s*([\d\.]+)\s*<\/td>\s*<td[^>]*>\s*([\d\.]+)\s*<\/td>/gi,
		names: ['currency', 'multiplier', 'buy', 'sale'],
		headers: headers,
		dateReg: /<td class="blueRow" colspan="2">(\d+\.\d{1,2}.\d{4})<\/td>/gi,
		dateParser: function (date) {
			date = date.split(/\./g);
			date.reverse();
			if (date[1].length === 1) {
