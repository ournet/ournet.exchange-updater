	...
	return Promise.resolve(sources)
		.map(function(source) {
			return preSource(country, source).then(function() {
				debug('starting explore source:', source.name || source.source);
				var extract = source.extractor || htmlExtract;
				return extract(source.options)
					.then(function(rates) {
						rates = rates || [];
						debug('done explore source:', source.name || source.source);
						...
						if (rates.length === 0) {
							logger.warn('No rates for source: ' + source.options.url);
						}
						return rates;
